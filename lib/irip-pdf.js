...
const BRAND_IMAGE_PATHS = {
  headerStrip: path.join(process.cwd(), 'public', 'irip', 'header-strip.png'),
  footerStrip: path.join(process.cwd(), 'public', 'irip', 'footer-strip.png')
};
...
async function loadTemplateAssets() {
  if (!templateAssetsPromise) {
    templateAssetsPromise = (async () => {
      ...
      for (const [key, assetPath] of Object.entries(BRAND_IMAGE_PATHS)) {
        const assetBuffer = await fs.readFile(assetPath);
        const jpegBuffer = await sharp(assetBuffer)
...
function drawBrandHeader(page) {
  page.drawImage('headerStrip', 0, 0, PAGE_WIDTH, BRAND_HEADER_BOTTOM);
}
...
function drawFooter(page) {
  page.drawImage('footerStrip', 0, FOOTER_TOP, PAGE_WIDTH, PAGE_HEIGHT - FOOTER_TOP);
}
