...
async function buildImageComposites(assets) {
  const composites = [];
  ...
  for (const placement of PAGE_IMAGE_PLACEMENTS) {
    ...
    const input = await sharp(asset.buffer)
      .resize({
        width: Math.max(1, Math.round(placement.width * RENDER_SCALE)),
        height: Math.max(1, Math.round(placement.height * RENDER_SCALE)),
        fit: 'contain',
    ...
    composites.push({
      input,
      left: Math.round(placement.x * RENDER_SCALE),
      top: Math.round(placement.y * RENDER_SCALE)
    });
  }
...
async function renderSvgPageToJpeg(svg, assets) {
  const basePage = await sharp(Buffer.from(svg))
    .png()
    .toBuffer();

  const composites = await buildImageComposites(assets);

  return sharp(basePage)
...
export async function generateIripPdfBuffer({ learnerName, gradeLevel, tutorName, rows }) {
  ...
  for (const page of pages) {
    ...
    renderedPages.push(await renderSvgPageToJpeg(svg, assets));
  }
