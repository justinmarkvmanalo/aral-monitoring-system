...
const FIRST_PAGE_TABLE_TOP = FORM_HEADER_TOP + FORM_HEADER_HEIGHT + 8;
const CONTINUATION_TABLE_TOP = BRAND_HEADER_BOTTOM + 12;
...
const PAGE_IMAGE_PLACEMENTS = [
  { key: 'headerSeal', x: PAGE_WIDTH / 2 - 12, y: 4, width: 24, height: 24 },
  { key: 'schoolSeal', x: 34, y: FOOTER_TOP + 7, width: 24, height: 24 },
  { key: 'facebook', x: 72, y: FOOTER_TOP + 26, width: 8, height: 8 },
  { key: 'quezon', x: 162, y: FOOTER_TOP + 8, width: 70, height: 22 },
  { key: 'website', x: 214, y: FOOTER_TOP + 26, width: 8, height: 8 },
  { key: 'agdangan', x: 314, y: FOOTER_TOP + 6, width: 112, height: 34 },
  { key: 'serbisyo', x: 360, y: FOOTER_TOP + 33, width: 122, height: 24 },
  { key: 'matatag', x: 618, y: FOOTER_TOP + 10, width: 94, height: 24 },
  { key: 'email', x: 704, y: FOOTER_TOP + 27, width: 9, height: 9 }
];
...
function getTableTop(isFirstPage) {
  return isFirstPage ? FIRST_PAGE_TABLE_TOP : CONTINUATION_TABLE_TOP;
}

function getPageContentStartY(isFirstPage) {
  return getTableTop(isFirstPage) + TABLE_HEADER_HEIGHT;
}
...
function buildPageLayout(groups) {
  ...
  let currentY = getPageContentStartY(true);
  const contentBottom = FOOTER_TOP - 8;

  for (const group of groups) {
    if (currentY + group.totalHeight > contentBottom && currentPage.groups.length > 0) {
      ...
      currentY = getPageContentStartY(false);
    }
  ...
  let lastPageY = getPageContentStartY(lastPage.isFirstPage) +
    lastPage.groups.reduce((sum, group) => sum + group.totalHeight, 0);

  if (lastPageY + LEGEND_BLOCK_HEIGHT > contentBottom) {
    lastPage = {
      groups: [],
      includeLegend: true,
      isFirstPage: false
...
function drawBrandHeader(page) {
  page.drawImage('headerSeal', PAGE_WIDTH / 2 - 12, 4, 24, 24);
  page.drawText('Republic of the Philippines', 0, 26, {
    font: 'F3',
    fontSize: 11,
    align: 'center',
    width: PAGE_WIDTH
  });
  page.drawText('Department of Education', 0, 39, {
    font: 'F3',
    fontSize: 15.5,
    align: 'center',
    width: PAGE_WIDTH
  });
  page.drawText('Region IV-A', 0, 53, {
    font: 'F2',
    fontSize: 10,
    align: 'center',
    width: PAGE_WIDTH
  });
  page.drawText('SCHOOLS DIVISION OF QUEZON PROVINCE', 0, 63, {
    font: 'F2',
    fontSize: 10.2,
    align: 'center',
    width: PAGE_WIDTH
  });
  page.drawText('AGDANGAN CENTRAL ELEMENTARY SCHOOL', 0, 74, {
    font: 'F2',
    fontSize: 10.2,
    align: 'center',
    width: PAGE_WIDTH
  });
  page.drawLine(PAGE_MARGIN_X, 82, PAGE_WIDTH - PAGE_MARGIN_X, 82, { lineWidth: 1.2 });
  page.drawText('Poblacion I, Agdangan, Quezon', 0, 84, {
    font: 'F2',
    fontSize: 9.2,
    align: 'center',
    width: PAGE_WIDTH,
...
function drawTableHeader(page, columnWidths, tableTop) {
  const labels = ['WEEK', 'READING SUBSKILL WEEKLY', 'STATUS', 'TUTOR NOTES/ OBSERVATIONS'];
  let cursorX = PAGE_MARGIN_X;

  labels.forEach((label, index) => {
    const width = columnWidths[index];
    page.drawRect(cursorX, tableTop, width, TABLE_HEADER_HEIGHT, {
      fill: true,
      stroke: true,
      fillColor: [242, 242, 242],
      lineWidth: 0.8
    });
    page.drawText(label, cursorX + 2, tableTop + 6.5, {
      font: 'F2',
...
function drawFooter(page) {
  ...
  page.drawText('DepEd Tayo Agdangan CES', 84, FOOTER_TOP + 22, {
    font: 'F2',
    fontSize: 6.4
  });
  page.drawText('https://agdangances.weebly.com', 226, FOOTER_TOP + 22, {
    fontSize: 6.0
  });
  page.drawText('Lead the Race.... ACES', 362, FOOTER_TOP + 2, {
    font: 'F2',
    fontSize: 7.6,
    color: [68, 114, 196]
  });
  page.drawText('Address: Brgy. Poblacion I, Agdangan, Quezon', 618, FOOTER_TOP + 3, {
    fontSize: 5.4
  });
  page.drawText('Contact Numbers: 09104490517, (042) 785-0308', 618, FOOTER_TOP + 11, {
    fontSize: 5.4
  });
  page.drawText('108944@deped.gov.ph', 716, FOOTER_TOP + 22, {
    fontSize: 6.0
  });
}
...
export async function generateIripPdfBuffer({ learnerName, gradeLevel, tutorName, rows }) {
  ...
  pageLayout.forEach((pageLayoutItem) => {
    ...
    const tableTop = getTableTop(pageLayoutItem.isFirstPage);
    drawBrandHeader(page);
    if (pageLayoutItem.isFirstPage) {
      drawFormHeader(page, { learnerName, gradeLevel, tutorName });
    }
    drawTableHeader(page, columnWidths, tableTop);

    let currentY = tableTop + TABLE_HEADER_HEIGHT;
