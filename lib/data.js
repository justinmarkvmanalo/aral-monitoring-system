...
export async function getTeacherDashboardData(teacherId) {
  ...
  const [
    ...
    getTeacherComprehensionAssessments(section.id)
  ]);
...
export async function getTeacherComprehensionAssessments(sectionId) {
  try {
    const result = await query(
      `select ca.id, ca.student_id, ca.assessed_date, ca.passage_title,
              ca.total_questions, ca.correct_answers, ca.score, ca.level, ca.notes,
              ca.created_at, s.first_name, s.last_name
         from comprehension_assessments ca
         inner join students s on s.id = ca.student_id
        where s.section_id = $1
        order by ca.assessed_date desc, ca.created_at desc`,
      [sectionId]
    );
    return result.rows;
  } catch (error) {
    if (isMissingRelation(error, 'comprehension_assessments') || isMissingColumn(error)) {
      return [];
    }
    throw error;
  }
...
export async function saveComprehensionAssessment({
  studentId,
  assessedDate,
  passageTitle,
  totalQuestions,
  ...
  return query(
    `insert into comprehension_assessments (student_id, assessed_date, passage_title, total_questions, correct_answers, score, level, notes, recorded_by)
     values ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
    [studentId, assessedDate, passageTitle, totalQuestions, correctAnswers, score, level, notes, teacherId]
  );
}
...
export async function getPhilIriQstData(sectionId) {
  const students = await getTeacherStudents(sectionId);

  const [readingAssessments, comprehensionAssessments] = await Promise.all([
    query(
      `select distinct on (rl.student_id) rl.student_id, rl.level, rl.comprehension_pct, rl.pronunciation
         from reading_levels rl
        inner join students s on s.id = rl.student_id
        where s.section_id = $1
        order by rl.student_id, rl.assessed_date desc, rl.id desc`,
    ...
    query(
      `select distinct on (ca.student_id) ca.student_id, ca.score, ca.level
         from comprehension_assessments ca
        inner join students s on s.id = ca.student_id
        where s.section_id = $1
        order by ca.student_id, ca.assessed_date desc, ca.id desc`,
      [sectionId]
    )
  ]);

  const readingMap = new Map(readingAssessments.rows.map((r) => [r.student_id, r]));
  const comprehensionMap = new Map(comprehensionAssessments.rows.map((r) => [r.student_id, r]));

  const male = { enrollment: 0, oralFrustration: 0, oralInstructional: 0, oralIndependent: 0, compFrustration: 0, compInstructional: 0, compIndependent: 0, readingFrustration: 0, readingInstructional: 0, readingIndependent: 0 };
  const female = { enrollment: 0, oralFrustration: 0, oralInstructional: 0, oralIndependent: 0, compFrustration: 0, compInstructional: 0, compIndependent: 0, readingFrustration: 0, readingInstructional: 0, readingIndependent: 0 };
  ...
  for (const student of students) {
    ...
    if (reading) {
      if (reading.level === 'Frustration') gender.oralFrustration += 1;
      else if (reading.level === 'Instructional') gender.oralInstructional += 1;
      else if (reading.level === 'Independent') gender.oralIndependent += 1;
    }

    const comprehension = comprehensionMap.get(student.id);
    if (comprehension) {
      if (comprehension.level === 'Frustration') gender.compFrustration += 1;
      else if (comprehension.level === 'Instructional') gender.compInstructional += 1;
      else if (comprehension.level === 'Independent') gender.compIndependent += 1;
    }

    const oralLevel = reading?.level || null;
    const compLevel = comprehension?.level || null;

    if (oralLevel === 'Frustration' || compLevel === 'Frustration') {
      gender.readingFrustration += 1;
    } else if (oralLevel === 'Independent' && compLevel === 'Independent') {
      gender.readingIndependent += 1;
    } else if (oralLevel || compLevel) {
      gender.readingInstructional += 1;
    }
  }
