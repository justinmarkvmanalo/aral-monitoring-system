...
export async function getTeacherDashboardData(teacherId) {
  ...
  const attendanceConfig = buildAttendanceWeeks(section?.school_year_start_date);
...
export async function getAdminDashboardData() {
  const [teachers, sectionsRes, totalStudentsRes, announcementsRes] = await Promise.all([
...
export async function getTeacherIripDocumentData(teacherId, studentId) {
  ...
  return {
    ...
    rows: Array.isArray(iripRecord?.rows) ? iripRecord.rows : []
  };
}
...
function buildAttendanceWeeks(startDateValue, totalWeeks = 10) {
  const startDate = parseDateValue(startDateValue) || parseDateValue(getCurrentDateValue());
  ...
  for (let weekIndex = 0; weekIndex < totalWeeks; weekIndex += 1) {
    ...
    const dates = Array.from({ length: 5 }, (_, dayIndex) => formatDateValue(addDays(weekStart, dayIndex)));
    weeks.push({
      index: weekIndex,
  ...
  const currentWeekIndex = Math.max(
    0,
    weeks.findIndex((week) => week.dates.includes(today))
  );
  const sessionDates = [...new Set([...weeks.flatMap((week) => week.dates), today])];
...
function startOfWeek(date) {
  ...
  workingDate.setUTCDate(workingDate.getUTCDate() + mondayOffset);
  return workingDate;
}
