...
function getTimeoutValue(name, fallback) {
...
export function getPool() {
  if (!globalForDb.__aralDbPool) {
    globalForDb.__aralDbPool = new Pool({
      ...getConnectionConfig(),
      max: getPoolMax(),
      idleTimeoutMillis: getTimeoutValue('DB_IDLE_TIMEOUT_MS', 10000),
      connectionTimeoutMillis: getTimeoutValue('DB_CONNECTION_TIMEOUT_MS', 10000),
      query_timeout: getTimeoutValue('DB_QUERY_TIMEOUT_MS', 15000),
      statement_timeout: getTimeoutValue('DB_STATEMENT_TIMEOUT_MS', 15000),
      idle_in_transaction_session_timeout: getTimeoutValue('DB_TRANSACTION_TIMEOUT_MS', 15000),
