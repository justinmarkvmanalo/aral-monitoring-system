...
function fillHeader(xml, { learnerName, gradeLevel, tutorName }) {
  const learnerParagraphPattern =
    /<w:p>[\s\S]*?<w:t>Learner(?:â€™|’|'|&#8217;)s Name:[\s\S]*?<w:tab\/><w:tab\/><w:t>Grade Level:[\s\S]*?<\/w:p>/;
  const tutorParagraphPattern =
    /<w:p>[\s\S]*?<w:t(?: xml:space="preserve")?>Tutor(?:â€™|’|'|&#8217;)s Name:\s*<\/w:t>[\s\S]*?<w:bookmarkStart[\s\S]*?<\/w:p>/;
