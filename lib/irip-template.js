...
function fillHeader(xml, { learnerName, gradeLevel, tutorName }) {
  const learnerLinePattern = /Learner(?:â€™|’|'|&#8217;)s Name:[^<]*Grade Level:[^<]*/;
  const tutorLinePattern = /Tutor(?:â€™|’|'|&#8217;)s Name:[^<]*/;

  xml = xml.replace(
    learnerLinePattern,
    escapeXml(`Learner's Name: ${learnerName || ''}  Grade Level: ${gradeLevel || ''}`)
  );
...
function fillChecklistTable(xml, rows) {
  ...
  const firstRowByWeek = new Set();
  rows.forEach((row, index) => {
    if (!firstRowByWeek.has(row.week)) {
      firstRowByWeek.add(row.week);
    }
  });
  ...
  const updatedTable = tableXml.replace(/<w:tr>[\s\S]*?<\/w:tr>/g, () => {
    const replacement = updatedRows[rowIndex];
    rowIndex += 1;
    return replacement;
  });
